...
from ..attention import SparseMultiHeadAttention4D, SerializeMode
...
class SparseTransformer4DBlock(nn.Module):
    """
    Sparse Transformer block (MSA + FFN).
    """
    def __init__(
        ...
        self.attn = SparseMultiHeadAttention4D(
            channels,
    ...
    def _forward(self, x: SparseTensor, num_frames: int) -> SparseTensor:
        h = x.replace(self.norm1(x.feats))
        h = self.attn(h, num_frames=num_frames)
        x = x + h
        h = x.replace(self.norm2(x.feats))
        h = self.mlp(h)
        x = x + h
    ...
    def forward(self, x: SparseTensor, num_frames: int) -> SparseTensor:
        if self.use_checkpoint:
            return torch.utils.checkpoint.checkpoint(self._forward, x, num_frames, use_reentrant=False)
        else:
            return self._forward(x, num_frames)
