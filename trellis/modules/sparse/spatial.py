...
class SparseDownsample_t(nn.Module):
    """
    Downsample a sparse tensor by a factor of `factor`.
    Implemented as average pooling.
    """
    def __init__(self, factor: Union[int, Tuple[int, ...], List[int]]):
        super(SparseDownsample_t, self).__init__()
        self.factor = tuple(factor) if isinstance(factor, (list, tuple)) else factor

    def forward(self, input: SparseTensor) -> SparseTensor:
        DIM = 1
        XYZ_DIM = input.coords.shape[-1] - 1
        factor = self.factor if isinstance(self.factor, tuple) else (self.factor,) * DIM
        assert DIM == len(factor), 'Input coordinates must have the same dimension as the downsample factor.'
        ...
        MAX = [coord[i+1].max().item() + 1 for i in range(XYZ_DIM)]
        OFFSET = torch.cumprod(torch.tensor(MAX[::-1]), 0).tolist()[::-1] + [1]
        code = sum([c * o for c, o in zip(coord, OFFSET)])
        code, idx = code.unique(return_inverse=True)
        new_feats = torch.scatter_reduce(
        ...
        new_coords = torch.stack(
            [code // OFFSET[0]] +
            [(code // OFFSET[i+1]) % MAX[i] for i in range(XYZ_DIM)],
            dim=-1
        )
        
        out = SparseTensor(new_feats, new_coords, (input.shape[0]//2,) + input.shape[1:],)
        out._scale = tuple([0,0,0])
        out._spatial_cache = input._spatial_cache

        out.register_spatial_cache(f'upsample_t_{factor}_coords', input.coords)
        out.register_spatial_cache(f'upsample_t_{factor}_layout', input.layout)
        out.register_spatial_cache(f'upsample_t_{factor}_idx', idx)

        return out


class SparseUpsample_t(nn.Module):
    """
    Upsample a sparse tensor by a factor of `factor`.
    Implemented as nearest neighbor interpolation.
    """
    def __init__(self, factor: Union[int, Tuple[int, int, int], List[int]]):
        super(SparseUpsample_t, self).__init__()
        self.factor = tuple(factor) if isinstance(factor, (list, tuple)) else factor
    ...
    def forward(self, input: SparseTensor) -> SparseTensor:
        ...
        new_coords = input.get_spatial_cache(f'upsample_t_{factor}_coords')
        new_layout = input.get_spatial_cache(f'upsample_t_{factor}_layout')
        idx = input.get_spatial_cache(f'upsample_t_{factor}_idx')
        
        if any([x is None for x in [new_coords, new_layout, idx]]):
            raise ValueError('Upsample cache not found. SparseUpsample must be paired with SparseDownsample.')
        new_feats = input.feats[idx]

        out = SparseTensor(new_feats, new_coords, (input.shape[0]*2,) + input.shape[1:], new_layout)
