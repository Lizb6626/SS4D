...
def rgb_to_srgb(f: torch.Tensor) -> torch.Tensor:
    ...
    f = torch.where(f > 0.0031308, torch.pow(torch.clamp(f, min=0.0031308), 1.0 / 2.4) * 1.055 - 0.055, 12.92 * f)
    return f

def srgb_to_rgb(f: torch.Tensor) -> torch.Tensor:
    # f is LDR
    f = torch.where(f <= 0.04045, f / 12.92, torch.pow((torch.clamp(f, min=0.04045) + 0.055) / 1.055, 2.4))
    return f
